            ...
            <div className="sm:col-span-4 flex flex-col items-start justify-center space-y-3">
              ...
              <div className="flex items-center space-x-4 flex-wrap justify-start">
                {info?.socials && info?.socials.map((social) => (
                  <a key={social?.name} target="_blank" rel="noreferrer" href={social?.url} className="inline-block my-1 bg-rose-500 text-white font-semibold py-2 px-3">{social?.name}</a>
                ))}
              </div>
            </div>
          </div>
        ...
        <section className="mxw-sm my-12">
          ...
          <div className="flex items-center flex-wrap">
            {about?.skills?.map((skill) => (
              <span key={skill?.id} className="py-2 px-3 my-1 mx-2 shadow-lg shadow-slate-300 bg-rose-500 text-white font-semibold">{skill?.name}</span>
            ))}
          </div>
