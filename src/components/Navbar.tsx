      ...
      <nav className="mxw-lg flex items-center justify-between">
        ...
        <div className="space-x-1 md:space-x-4">   
          {links && links.map((link) => (
            <Link key={link.name} href={link?.route} className={router.pathname == link?.route ? "p-1 text-rose-600 border-2 border-transparent border-b-rose-600 text-sm md:text-md lg:text-lg" : "p-1 hover:text-rose-600 border-2 border-transparent hover:border-b-rose-600 duration-300 text-sm md:text-md lg:text-lg"}>{link?.name}</Link>
          ))}
        </div>
